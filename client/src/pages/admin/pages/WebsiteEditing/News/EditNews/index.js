...
const EditNews = () => {
  ...
  const onFinish = async (values) => {
    ...
    await http.put('/api/news/' + newsId, {
      title, description, category, datetime, media: uploadedUrl
    })
    navigate('/admin/websiteEditing/news')
  }
  ...
  const loadList = async () => {
    const res = await http.get('api/news/' + newsId)
    console.log(res)
    setInitialValues({ ...res.data })
    // setInitialValues({ "date": res.datetime })
